...
5. 源提示词只允许机器搬运（整文件复制/直链下载），严禁人肉转录；大批次走 `.url` 直链 + SHA-256（README §6）。
6. 严禁把 token / 密钥写入任何提交文件。
...
4. 跑不了 Python：把 9 字段 JSON 提交到 `待入库/`；大文件改提交两行 `.url` 文件（第 1 行直链、第 2 行 SHA-256）。CI 自动入库，结果看 Actions 绿勾与 `运行记录/最近入库.log`。

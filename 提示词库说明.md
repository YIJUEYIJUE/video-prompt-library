...
配套库文件：`视频提示词库-多模型-191条-V1.1.html`｜全库指纹 `14cdcb145f3ae309`｜2026-08-20
...
库文件命名：`视频提示词库-多模型-<N>条-V<主>.<次>.html` 作为**固定句柄**——文件名里的条数 / 版本标签可能陈旧，真实条数与指纹一律以库内 `meta.完整性指纹` 为准（脚本每次追加都只写回同一文件）。唯一数据源，数据、规则、指纹、界面全内嵌，双击即用。
...
- **V1.1**：交付规则细化（同一会话只交库文件；跨 AI 交接才交两份），数据与逐条指纹与 V1.0 完全一致；文件名 `视频提示词库-多模型-191条-V1.1.html` 即沿用为固定句柄，此后追加不再改名。
...
**第 1 步｜读库**：用 `库维护工具.py verify 视频提示词库-多模型-191条-V1.1.html` 确认当前条数、版本、全库指纹、有无异常；如需看规则 / 注册表，解析库内 meta。
...
**第 4 步｜校验**：`python 库维护工具.py verify 视频提示词库-多模型-191条-V1.1.html` 必须全绿（指纹、字段、标签、无重复）。
...
# 导出（目录要先建好，工具不自建目录）
mkdir -p 导出目录 && python 库维护工具.py export 视频提示词库-多模型-191条-V1.1.html 导出目录
```
